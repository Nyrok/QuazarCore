...
final class ReportCommand extends QuazarCommands
{
    ...
    public function reportPlayer(Player $player, Player $target): void {
        $form = new CustomForm('§m§a'.str_replace("{name}", $target->getName(), LanguageProvider::getLanguageMessage('forms.report.2.title', PlayerProvider::toQuazarPlayer($player), false)));
