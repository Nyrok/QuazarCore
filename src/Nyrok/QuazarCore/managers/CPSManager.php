...
abstract class CPSManager
{
    ...
    {
        if(!isset(self::$clicksData[mb_strtolower($player->getName())]) || empty(self::$clicksData[mb_strtolower($player->getName())])){
            return 0.0;
        }
        $ct = microtime(true);
        return round(count(array_filter(self::$clicksData[mb_strtolower($player->getName())], static function(float $t) use ($deltaTime, $ct) : bool{
